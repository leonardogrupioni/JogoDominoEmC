...
void acessarJogo1x1(PECA pecas[], PECA_MESA mesa[], CONTADOR * cont) {
    ...
    int primeiroAJogar;
    int voltarMenu;
    primeiroAJogar = conferirPecaInicial(pecas, mesa, &cont); //return + model

    imprimirJogadorInicial(primeiroAJogar); //view
    imprimirMesa(mesa, &cont); //view
    ...
    do{
        imprimirTurno(turno);
        imprimirMesa(mesa, &cont);
        switch (jogador) {
            case 1:
                do {
                    sinal = 1;
                    imprimrVezJogador(jogador);
                    imprimirMao(pecas, jogador, &cont);//
                    imprmirMenuJogando();
                    escolha = escolher();
                    switch (escolha){
                        case 1: //jogar
                            ...
                            pecaSelecionada = selecionarPeca(&cont, jogador);
        ...
        turno++;        
    } while(fimDeJogo(pecas, &cont, mesa) != 0 || voltarMenu != 1); //verif
